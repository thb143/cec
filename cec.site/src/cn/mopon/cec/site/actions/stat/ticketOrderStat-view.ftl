...
<@dwz.pageForm action="/stat/ticketOrderStat-view" targetType="dialog" alt="可输入订单号、影院订单号、渠道名称检索" >
    <@s.hidden path="searchModel.startDate" />
    <@s.hidden path="searchModel.endDate" />
    <@s.hidden path="searchModel.cinemaId" />
    <li>放映类型：<@s.select path="searchModel.showType" items=ShowType?values itemValue="value" itemLabel="text" class="required" headerLabel="全部"/></li>
</@dwz.pageForm>
</div>
<div class="pageContent">
    ...
    <div class="panelBar">
    <@dwz.pageNav pageModel=ticketOrderStatPage targetType="dialog"/>
    </div>
</div>
</div>
