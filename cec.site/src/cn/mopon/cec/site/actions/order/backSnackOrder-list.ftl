<div class="page">
    <div class="pageHeader">
        <@dwz.pageForm action="/order/backSnackOrder-list.ftl" alt="可输入卡号、渠道开卡订单号、手机号码检索">
        ...
        <table class="table" width="100%" layoutH="85">
            <thead>
                <tr>
                    ...
                    <th width="200" align="center">退订时间</th>
                    <th width="80" align="center">销售价</th>
                    <th width="80" align="center">门市价</th>
                    <th width="80" align="center">影院 结算价</th>
            ...
            <tbody>
            	<#list orderPages.contents as order>
	                <tr>
	                    ...
	                    <td>
	                    	<@dwz.a href="/order/snackOrder-view?snackOrder=${order.id}" target="dialog" title="查看订单" width="L" height="L">${order.voucher.code}</@dwz.a>
	                    </td>
	                    <td>${order.mobile}</td>
	                    <td>${order.cinema.name}</td>
	                    <td>${order.channel.name}</td>
	                    <td>${order.createTime?datetime}</td>
	                    <td>${order.revokeTime?datetime}</td>
