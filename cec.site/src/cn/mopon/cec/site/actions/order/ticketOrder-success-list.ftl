<div class="page">
    <div class="pageHeader">
        <@dwz.pageForm action="/order/ticketOrder-success-list" alt="可输入订单号、影院订单号、接入商、影院名称、影院排期编号、渠道排期编号、手机号码检索">
            ...
            <tbody>
            	<#list orderPages.contents as order>
	                <tr>
	                    ...
	                    <td>
		                    <@sec.any name="ORDER_MANAGE">
		                        <@dwz.a href="/order/ticketOrder-sync?ticketOrder=${order.id}" target="dialog" height="SS">同步影院订单</@dwz.a>
		                        <#if order.voucher.isResetable()>
	                            	<@dwz.a href="/order/ticketVoucher-reset?voucherId=${order.voucher.id}" target="ajaxTodo" title="您是否确定要重置凭证？">重置凭证</@dwz.a>
	                         	</#if>
		                    </@sec.any>
	                    </td>
	                </tr>
