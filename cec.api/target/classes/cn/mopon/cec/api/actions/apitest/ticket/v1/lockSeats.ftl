<div class="page" layouth="0">
    <div class="pageFormContent">
	    <form class="pageForm required-validate" action="${ctx}/api/ticket/v1/lockSeats">
	        <fieldset>
	            <legend>锁座下单接口</legend>
	            <dl>
	                <dt>渠道编号:</dt>
	                <dd><input type="text" name="channelCode" class="required" value="0003"/></dd>
	            </dl>
	            <dl>
	                <dt>渠道排期编码:</dt>
	                <dd><input type="text" name="channelShowCode" class="required" value="1000414052905411"/></dd>
	            </dl>
	            <dl>
	                <dt>电影票结算价:</dt>
	                <dd><input type="text" name="submitPrice" value="30"/></dd>
	            </dl>
	            <dl>
	                <dt>渠道订单号:</dt>
	                <dd><input type="text" name="channelOrderCode" class="required" value="1000414052905411"/></dd>
	            </dl>
	            <dl>
	                <dt>签名:</dt>
	                <dd><input type="text" name="sign" class="required" value="0003"/></dd>
	            </dl>
	            <dl class="nowrap">
	                <dt>座位列表(,分隔):</dt>
	                <dd><input type="text" name="seatCodes" class="required" size="90"/></dd>
	            </dl>
	            <button type="button" queryType="json" onclick="submitJsonQuery(this)">返回JSON</button>
	            <button type="button" queryType="xml" onclick="submitJsonQuery(this)">返回XML</button>
	        </fieldset>
	    </form>
    </div>
    <div layouth="120"></div>
</div>
